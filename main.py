...
async def get_candles(session, ticker, days=2):
    """Получение свечей - ТОЧНО как в первом боте"""
    ...
    # Создаем DataFrame как в первом боте
    ...
    df = df.tail(CANDLES)  # Берем последние CANDLES свечей
...
def calculate_liquidity_levels(df):
    """Уровни ликвидности (20 периодов)"""
    try:
...
def detect_liquidity_grab(df):
    ...
    try:
        ...
        # SHORT: пробой high и возврат ниже
        if prev["high"] > high_lvl and last["close"] < prev["high"]:
            return "SHORT"
        
        # LONG: пробой low и возврат выше
...
def generate_trading_signal(df, bid_vol, ask_vol, ticker=""):
    ...
    logger.info(f"🔍 {ticker}: Захват ликвидности {side}")
    ...
    logger.info(f"🔍 {ticker}: Импульс OK")
    ...
    if dist_low >= 0.02 and dist_high >= 0.02:
        logger.debug(f"❌ {ticker}: Цена далеко от FVG")
        return None
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🚀 <b>SMC Trading Bot v6.0</b>\n\n"
        f"📊 Тикеров: {len(TICKERS)}\n"
        f"⏱ Интервал: {INTERVAL} мин\n"
        "📋 /status /tickers /signals /help",
        parse_mode='HTML'
    )
...
async def signals_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not recent:
        await update.message.reply_text("📊 Нет сигналов за час")
        return
    
    text = f"📊 <b>Сигналы ({len(recent)}):</b>\n\n"
    for key, ts in sorted(recent.items(), key=lambda x: x[1], reverse=True)[:10]:
        parts = key.split('_')
        if len(parts) >= 2:
            emoji = "🟢" if parts[1] == "LONG" else "🔴"
            text += f"{emoji} {parts[0]} - {parts[1]}\n"
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        logger.info("=" * 60)
        logger.info("🚀 SMC Trading Bot v6.0")
        logger.info(f"📊 {len(TICKERS)} тикеров")
        ...
        while True:
            try:
                ...
                logger.info(f"✅ Цикл #{total_cycles}: завершен")
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(f"Критическая ошибка: {e}", exc_info=True)import asyncio
import aiohttp
import pandas as pd
import os
import time
...
def generate_trading_signal(df, bid_vol, ask_vol, ticker=""):
    """Генерация сигнала с подсчетом статистики отказов"""
    if df is None or len(df) < 50:
        return None
...
async def send_hourly_status(bot):
    """Отправка ежечасного отчета в Telegram"""
    global last_status_time
    
    current_time = time.time()
    if current_time - last_status_time < 3600:  # Каждый час
        return
    ...
    try:
        ...
        message = (
            ...
            f"2️⃣ Импульс (Displacement): {steps_failed['step2']}\n"
            f"3️⃣ Всплеск объема: {steps_failed['step3']}\n"
            f"4️⃣ Fair Value Gap: {steps_failed['step4']}\n"
            f"5️⃣ Стакан: {steps_failed['step5']}\n"
            f"6️⃣ Далеко от FVG: {steps_failed['step6']}\n"
            f"📊 Всего отказов: {total_fails}"
        )
        
        await bot.send_message(chat_id=CHAT_ID, text=message, parse_mode='HTML')
        logger.info("📤 Ежечасный отчет отправлен в Telegram")
...
async def test_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Тестовая команда - показывает где отсеиваются сигналы"""
    total_fails = sum(steps_failed.values())
    
    if total_fails == 0:
        status = "✅ Все шаги пройдены (нет отказов)"
    else:
        status = (
            f"1️⃣ Захват ликвидности: {steps_failed['step1']}\n"
    ...
    await update.message.reply_text(
        ...
        f"<b>Где отсеиваются сигналы:</b>\n"
        f"{status}\n\n"
        f"💡 <i>Если много отказов на шаге 1-3,\n"
        f"попробуйте смягчить множители (1.5x → 1.2x)</i>",
        parse_mode='HTML'
    )
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "2️⃣ Импульсное движение (1.5x)\n"
        "3️⃣ Всплеск объема (1.5x)\n"
        "4️⃣ Fair Value Gap (3 свечи)\n"
        "5️⃣ Анализ стакана (BID/ASK)\n"
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        logger.info(f"⏱ Интервал: {INTERVAL} мин")
        logger.info(f"📋 Команды: /start /status /test /tickers /signals /help")
        logger.info(f"📊 Ежечасный отчет: ВКЛ")
        logger.info("=" * 60)
        ...
        try:
            await bot.send_message(
                ...
                text="✅ <b>Бот v7.0 запущен!</b>\n"
                     ...
                     f"⏱ Интервал: {INTERVAL} мин\n"
                     f"📊 Ежечасные отчеты включены\n\n"
                     "<i>/test - посмотреть статистику отказов</i>",
                parse_mode='HTML'
            )
        except:
            pass
        ...
        while True:
            try:
                ...
                logger.info(f"✅ Цикл #{total_cycles}: завершен | Данных: {ticks_with_data} | Без данных: {ticks_without_data} | Сигналов: {signals_found}")
